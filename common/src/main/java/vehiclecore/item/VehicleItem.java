...
public class VehicleItem extends DescriptionItem {
    ...
    public InteractionResultHolder<ItemStack> use(Level world, Player user, InteractionHand hand) {
        ...
        if (((HitResult) hitResult).getType() == HitResult.Type.MISS) {
            error(user, "vehiclecore.tooltip.no_target");
        ...
        if (((HitResult) hitResult).getType() == HitResult.Type.BLOCK) {
            ...
            if (!world.noCollision(entity, entity.getBoundingBox())) {
                error(user, "vehiclecore.tooltip.no_space");
    ...
    public void appendHoverText(ItemStack stack, @Nullable Level world, List<Component> tooltip, TooltipFlag context) {
        ...
        if (tag != null) {
            if (tag.contains("Inventory")) {
                ...
                tooltip.add(Component.translatable("vehiclecore.tooltip.inventory", nbtList.size()));
            }
        }
    }
}
