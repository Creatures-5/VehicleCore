...
import vehiclecore.network.s2c.AircraftDataMessage;
import vehiclecore.network.s2c.VehicleUpgradesMessage;
...
public class ForgeBusEvents {
    ...
    public static void onItemTooltips(ItemTooltipEvent event) {
        ...
        if (upgrade != null) {
            ...
            tooltip.add(Component.translatable("item.vehiclecore.item.upgrade").withStyle(ChatFormatting.GRAY).withStyle(ChatFormatting.ITALIC));

            for (Map.Entry<VehicleStat, Float> entry : upgrade.getAll().entrySet()) {
                tooltip.add(Component.translatable("vehiclecore.upgrade." + entry.getKey().name().toLowerCase(Locale.ROOT),
