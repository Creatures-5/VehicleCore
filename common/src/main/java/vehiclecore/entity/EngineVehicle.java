                ...
                if (getControllingPassenger() instanceof ServerPlayer player) {
                    ...
                    if (utilization > 0 && isFuelLow()) {
                        if (lastFuelState != FuelState.LOW) {
                            player.displayClientMessage(Component.translatable("vehiclecore." + getFuelType() + ".low"), true);
                        ...
                        if (lastFuelState != FuelState.EMPTY) {
                            player.displayClientMessage(Component.translatable("vehiclecore." + getFuelType() + "." + (lastFuelState == FuelState.FUELED ? "out" : "none")), true);
